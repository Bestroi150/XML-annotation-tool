...
class Example(Frame):
    def __init__(self, parent):
        ...
        self.parent = parent
        self.s = ttk.Separator(parent, orient='horizontal')
        self.fileName = ""
        self.debug = False
        self.colorAllChunk = True
        self.history = deque(maxlen=20)
        self.currentContent = deque(maxlen=1)
        self.pressCommand = {'a': u"enterValue",
                             'b': u"enterValue",
                             'c': u"enterValue",
                             'd': u"enterValue",
                             'e': u"enterValue",
        ...
        #mine self.entityRe=r'<(.*)/>'
        #mine self.insideNestEntityRe=r'<(.*)/>'
        #kinda work self.entityRe = r"^<.*>.*</.*>$"
        #self.insideNestEntityRe = r"^<.*>.*</.*>$"
        #self.entityRe = r'\[\@.*?\#.*?\*\](?!\#)'
        #self.entityRe = r'\<.*?\>.*?\<\/.*?\>' restore if something doesnt work
        self.entityRe = r"\<.*?\>.*?\<\/.*?[^\=][^\"\"]\>"
        #self.entityRe = r'^.* <.*>.*</.*>.*$'
        #self.insideNestEntityRe= r'<.*>[a-zA-Z]+</.*>'
        #r'\[\@\[\@(?!\[\@).*?\#.*?\*\]\#'
        #r' \<.*?\<.*?\<.*?(?!\<#).*?>/.*?>\#'
        #r'\<.*?\>.*?\<\/.*?\>(?!\#)'
        #self.insideNestEntityRe = r'\[\@\[\@(?!\[\@).*?\#.*?\*\]\#'
        self.insideNestEntityColor = r"\<.*?\>.*?\<\/.*?[^\=][^\"\"]\>"
        ## configure color#"navajo white"#"DeepSkyBlue3","peach puff"
        #self.entityColor= []             
        #self.entityColor= ("navajo white","peach puff","DeepSkyBlue3")
        self.insideNestEntityColor = "green"
        self.selectColor = 'light green'
        self.maxEventId = 0
        self.currentEventId = ""
        self.textFontStyle = "Lato"
    ...
    def initUI(self):
        ...
        self.parent.wm_attributes('-transparent', 'green')
        ...
        self.lbl = Label(self, text="File: no file opened")
        self.lbl.grid(sticky=W, pady=4, padx=5)
        self.fnt = tkFont.Font(font=(self.textFontStyle, 14, "bold"), size= self.textRow, underline=0)
        self.text = Text(self, font=self.fnt, selectbackground=self.selectColor)
        self.text.grid(row=1, column=0, columnspan=self.textColumn, rowspan=self.textRow, padx=12, pady=12,
                      sticky=E + W + S + N)
        self.text.columnconfigure(0, weight=1)
        self.text.rowconfigure(0, weight=1)
        self.text.columnconfigure(0, weight=3)
        self.text.columnconfigure(1, weight=3)
        self.text.columnconfigure(2, weight=3)
        self.text.columnconfigure(3, weight=1)
        self.text.columnconfigure(4, weight=1)
        self.text.rowconfigure(1, weight=1)

    


        self.scale = ttk.Scale(self, orient='horizontal', length=200, from_=10, to=30)
        self.scale.grid(sticky=W, pady=4, padx=5)
        self.scale.config(command=lambda e: self.fontSize().set)
        ...
        mbtn = ttk.Menubutton(self, text="File operations")
        mbtn.grid()
        mbtn.menu = Menu(mbtn,tearoff=0)
        mbtn["menu"] = mbtn.menu
        mbtn.grid(row=1, column=self.textColumn + 4, columnspan=2, rowspan=1, padx=10)
        

        mbtn.menu.add_radiobutton(label="Open", command=self.onOpen)
        mbtn.menu.add_radiobutton(label="Remap", command=self.renewPressCommand)
        mbtn.menu.add_radiobutton(label="Export File", command=self.generateSequenceFile)
        mbtn.menu.add_radiobutton(label="Export File as HTML", command=self.convertHtml) 
        mbtn.menu.add_radiobutton(label="Export File as XML", command=self.xml_print)        
        mbtn.menu.add_radiobutton(label="Quit", command=self.quit)
        ...
        self.selected_color.trace("w", self.menu_color_selected)
      
        

        menubutton = Menubutton(self, text = "Functions")    
        menubutton.grid(row=2, column=self.textColumn + 4, columnspan=2, rowspan=1, padx=10)  
        menubutton.menu = Menu(menubutton,tearoff=0)    
        menubutton["menu"]=menubutton.menu    
        menubutton.menu.add_radiobutton(label = "Open notes tab", command=self.notes)    
        menubutton.menu.add_radiobutton(label = "Open Google search", command=self.note)
        menubutton.menu.add_radiobutton(label = "Show Keybindings", command=self.keyBindings)        
        menubutton.menu.add_radiobutton(label = "Close Keybindings",command=self.keyDestroy)

        #show_tags_var = BooleanVar(self, True)
        #check = Checkbutton(self, text='Show XML Tags', variable=show_tags_var,
        #                    command=lambda: self.text.show_annotation_tag(show_tags_var.get()))
        #check.grid(row=9, column=self.textColumn + 1, sticky=W)
        ...
        self.EventName = Label(self, text="", foreground="", font=(self.textFontStyle, 14, "bold"))
        self.EventName.grid(row=8, column=self.textColumn + 1, pady=4)
        self.EventId = Label(self, textvariable=self.EventIdString, foreground="",
                             font=(self.textFontStyle, 14, "bold"))
        
        self.EventIdString.set("MaxId: %s\nCurId: %s" % (self.maxEventId, self.currentEventId))       
        self.cursorName = Label(self, text="", foreground="blue", font=(self.textFontStyle, 14, "bold"))
        #self.cursorName.grid(row=12, column=self.textColumn + 1, pady=4)
        self.cursorIndex = Label(self, text="", foreground="", font=(self.textFontStyle, 14, "bold"))
    ...
    def notes(self):
        ...
        entryNotes = Text(win,wrap= WORD, foreground="black",highlightbackground="black", selectbackground="navajo white", font=("Lato", 12, "roman"))
        entryNotes.pack(expand= True, fill= 'both')
        
        def saveNotes():
            text_file = open("test.txt", "w")
            text_file.write(entryNotes.get(1.0, END))
            text_file.close()    

        menu_bar = Menu(win)
        file_menu = Menu(menu_bar, tearoff= False)
        file_menu.add_command(label= 'Save',command=saveNotes)
        menu_bar.add_cascade(label= 'File', menu= file_menu)
        win.config(menu = menu_bar)
                     
         
        style.theme_use('equilux') #'equilux','breeze'
    ...
    def note(self):
        self.frame = webview.create_window('Ann Search Engine',"https://www.google.com/")        
        self.frame = webview.start()
        self.frame.grid(row=11, column=self.textColumn + 3, columnspan=4, rowspan=3, padx=4, )
        self.frame.grid.columnconfigure(tuple(range(60)), weight=1)
        self.frame.grid.rowconfigure(tuple(range(30)), weight=1)
    ...
    def fontSize(self):
        self.text.config(font=(self.fnt, int(self.scale.get())))


    def menu_color_selected(self, *args):
        """ handle menu selected event """
        self.config(bg=self.selected_color.get())     
    
    def changing_color(self):    
        
        entityColor= ('yellow','lime','light salmon','navajo white','peach puff','DeepSkyBlue3','LightSkyBlue1','#CCCCFF','#9FE2BF','#DE3163','#C0C0C0','#FFFFFF')
            
        btn = ttk.Menubutton(self, text="Entity Color")
        btn.grid(row=3, column=self.textColumn + 4, columnspan=1, rowspan=1)
        btn.menu = tk.Menu(btn,tearoff=0)
        btn["menu"] = btn.menu
        self.select_color = tk.StringVar()
        for color_name in entityColor:
            btn.menu.add_radiobutton(label=color_name, value=color_name, variable=self.select_color, command=self.change_color)
      
    def change_color(self):
        self.colorEntity = self.select_color.get()
    ...
    def onOpen(self):
        ftypes = [('all files', '.*'), ('text files', '.txt'), ('ann files', '.ann'),('doc files','.docx')]
    ...
    def executeCursorCommand(self, command):
        ...
        try:
            ...
            if re.match(self.entityRe, selected_string) != None:
                ...
                #OG new_string_list = selected_string.strip('[@]').rsplit('#', 1)
            ...
            if command == "q":
                ...
                if len(selected_string) > 0:
                    followHalf_content, cursor_index = self.replaceString(followHalf_content, selected_string, command,
                                                                          cursor_index)
                ...
                if command == "q":
                    ...
                    if len(selected_string) > 0:
                        if command in self.pressCommand:
                            line_after_entity, cursor_index = self.replaceString(line_after_entity, selected_string,
    ...
    def replaceString(self, content, string, replaceType, cursor_index):
        if replaceType in self.pressCommand:
            #og new_string = "[@" + string + "#" + self.currentEventId + self.pressCommand[replaceType] + "*]"
            new_string = "<" + self.pressCommand[replaceType]+">"+ string + self.currentEventId + "<" + "/" + re.sub('\s+[A-Za-z]+="[A-Za-z0-9]+"', "", self.pressCommand[replaceType]) + ">"
            #new_string = "<" + self.pressCommand[replaceType]+ ">"+ string  + self.currentEventId + "</" + self.pressCommand[replaceType] + ">"
...
#fuck me if it works
    ...
    #        self.entityColor += [c + n for c, n in zip([color] * 5, ['', '1', '2', '3', '4'])]
    #        return sorted (self.entityColor, key=lambda c: list(reversed(c)))   
    ...
    def setColorDisplay(self):
        ...
        while True: 
            #for key, color in zip(self.pressCommand.keys(), self.colors()):
            #може и да сработи
            #    key.color = color    
            #    self.colors = color    
            
            
            #if self.change_theme=='alt':       
            #    self.text.tag_configure("catagory", background=self.entityColor[2])
            #    self.text.tag_configure("edge", background=self.entityColor[2])  
            #    pos = self.text.search(self.entityRe, "matchEnd", "searchLimit", count=countVar, regexp=True)
            #else:
            #    self.text.tag_configure("catagory", background=self.entityColor[0])
            #    self.text.tag_configure("edge", background=self.entityColor[0])
            #    pos = self.text.search(self.entityRe, "matchEnd", "searchLimit", count=countVar, regexp=True)
            self.text.tag_configure("catagory", background=self.colorEntity)
            self.text.tag_configure("edge", background=self.colorEntity)  
            pos = self.text.search(self.entityRe, "matchEnd", "searchLimit", count=countVar, regexp=True)
    ...
    def setMapShow(self):
        ...
        width = 2
        row = 0        #A6F7D6
        mapLabel = Label(self, text="Shortcuts: ", foreground="#196F3D", font=(self.textFontStyle, 12, "bold"))
        mapLabel.grid(row=0, column=self.textColumn + 2, columnspan=2, rowspan=1, padx=12)
 
        
        #keyLabel2 = Label(self, text="""Undo = Ctrl+Z \nEsc = Exit fullscreen mode\nF11 = Enter fullscreen mode""", foreground="DarkOrange4", font=(self.textFontStyle, 10, "roman"))
        #keyLabel2.grid(row=6, column=self.textColumn + 4, columnspan=1, rowspan=1, padx=10)
        
    #!!restore    #mapLabel = Label(self, text="File operations ", foreground="dark green", font=(self.textFontStyle, 14, "bold"))
    #!!restore    #mapLabel.grid(row=0, column=self.textColumn + 4, columnspan=2, rowspan=1, padx=10)
        ...
        for key in sorted(self.pressCommand):
            ...
            symbolLabel = Label(self, text=key.upper() + ": ", foreground="#17A589", font=(self.textFontStyle, 12, "bold"))
            symbolLabel.grid(row=row, column=self.textColumn + 1, columnspan=1, rowspan=1, padx=3)
            self.shortcutLabelList.append(symbolLabel)
            symbolLabel.columnconfigure(0, weight=3)
            symbolLabel.columnconfigure(1, weight=3)
            symbolLabel.columnconfigure(2, weight=3)
            symbolLabel.columnconfigure(3, weight=1)
            symbolLabel.columnconfigure(4, weight=1)
            symbolLabel.rowconfigure(1, weight=1)

            labelEntry = Entry(self, foreground="#141010", font=(self.textFontStyle, 12, "bold"))
            ...
            labelEntry.columnconfigure(0, weight=3)
            labelEntry.columnconfigure(1, weight=3)
            labelEntry.columnconfigure(2, weight=3)
            labelEntry.columnconfigure(3, weight=1)
            labelEntry.columnconfigure(4, weight=1)
            labelEntry.rowconfigure(1, weight=1)


    def keyBindings(self):
        
        self.keyLabel1 = Label(self, text="Keybindings", foreground="#117864", font=(self.textFontStyle, 10, "bold"))
        self.keyLabel1.grid(row=4, column=self.textColumn + 4, columnspan=1, rowspan=1, padx=12)
        self.keyLabel2 = Label(self, text="Undo = Ctrl+Z", foreground="#117864", font=(self.textFontStyle, 10, "roman"))
        self.keyLabel2.grid(row=5, column=self.textColumn + 4, columnspan=1, rowspan=1, padx=12)
        self.keyLabel3 = Label(self, text="Esc = Exit fullscreen mode", foreground="#117864", font=(self.textFontStyle, 10, "roman"))
        self.keyLabel3.grid(row=6, column=self.textColumn + 4, columnspan=1, rowspan=1, padx=12)
        self.keyLabel4 = Label(self, text="F11 = Enter fullscreen mode", foreground="#117864", font=(self.textFontStyle, 10, "roman"))
        self.keyLabel4.grid(row=7, column=self.textColumn + 4, columnspan=1, rowspan=1, padx=12)    
        
    def keyDestroy(self):
        self.keyLabel1.destroy()
        self.keyLabel2.destroy()
        self.keyLabel3.destroy()
        self.keyLabel4.destroy()
    ...
    def generateSequenceFile(self):
        ...
        new_filename = self.fileName.split('.txt')[0] + '.txts'
    ...
    def convertHtml(self):
        ...
        self.doc.save("Annotation_output.html")       

    def markup(self,txt,paragraph_tag="p", inline_tags={"\n":"ln"}):
    ...
    def xml_print(self): 
        sample= self.text.get("1.0",END)
        value = self.markup(sample)
        
        with open("xmlOutput.xml", 'w', encoding='utf-8') as s:
            sys.stdout = s
            a = value
            for i in range(1):
...
def main():
    print("Annotation launched!")
    print(("OS:%s") % (platform.system()))
    #root = Tk()
    window  = TKMT.ThemedTKinterFrame("%test_theme.py","forest","light")#azure, sun-valley, park, forest
    w,h = window.root.winfo_screenwidth(), window.root.winfo_screenheight()
    window.root.geometry("%dx%d" % (w, h))
    window.root.attributes('-fullscreen', True)  
    window.root.bind("<F11>",lambda event: window.root.attributes("-fullscreen", not window.root.attributes("-fullscreen")))
    window.root.bind("<Escape>",lambda event: window.root.attributes('-fullscreen',False))         
    window.root.resizable(True, True)
  
    app = Example(window.root)
    app.setFont(17)
